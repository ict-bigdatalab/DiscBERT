...
def create_masked_lm_predictions(tokens, masked_lm_prob, max_predictions_per_seq, whole_word_mask, vocab_list, df_dict):
    ...
    cand_masked_sorted = sorted(cand_masked, key=lambda x: x.df, reverse = True)
    
    def from_cand(cand_masked, tokens):
        cand_indices = [cand.index for cand in cand_masked] ###
        masked_lms = []
        covered_indexes = set()
    ...
    tokens_masked_high_df = from_cand(cand_masked_sorted, tokens)
    
    return tokens_masked_high_df
